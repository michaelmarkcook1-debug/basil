...
export function ApproachChip({ hint, className }: { hint: ApproachHint; className?: string }) {
  return (
    <Link
      ...
      className={cn(
        "group relative inline-flex max-w-full items-start gap-1.5 rounded-lg border border-gold/20 bg-gold/[0.06] px-2.5 py-1.5",
        "text-xs leading-snug text-foreground/80 transition-colors hover:border-gold/40 hover:bg-gold/[0.10]",
        className
      )}
    >
      ...
      <Compass size={13} strokeWidth={1.8} className="mt-px shrink-0 text-gold" />
      <span className="min-w-0">
        <span className="font-semibold text-gold">{hint.name.split(" ")[0]}</span>
